...
const MenuCard = () => {
  ...
  const handleIncrement = async (index) => {
    ...
    const response=await axios.post("http://localhost:5000/api/1/order",{mode:"inc",idx:index});
  ...
  const handleDecrement = async (index) => {
    let prev=quantity[index];
    const response=await axios.post("http://localhost:5000/api/1/order",{mode:"dec",idx:index});
    console.log(response.data);
    const updatedQuantity = [...quantity];
  updatedQuantity[index] = response.data.qty;
